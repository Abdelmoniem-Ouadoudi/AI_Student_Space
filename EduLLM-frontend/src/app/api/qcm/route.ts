...
export async function POST(req: NextRequest) {
  ...
  // If no query is provided, use a default
  const actualQuery = query || "Generate questions based on the uploaded materials";
  ...
  if (!userId) {
    ...
    const enhancedQuery = `${actualQuery}\n\n${languageInstruction}`;
    ...
    if (!backendRes.ok) {
      ...
      if (backendRes.status === 0 || text.includes("ECONNREFUSED")) {
        return createMockQCMResponse(actualQuery, locale)
      }
    ...
    if (shouldGeneratePDF) {
      // Generate PDF
      const pdfBuffer = createPDFContent(aiResponse, `Generated quiz for: "${actualQuery}"`)
    ...
    const qcmData = {
      ...
      description: `Generated quiz for: "${actualQuery}"`,
      content: aiResponse,
