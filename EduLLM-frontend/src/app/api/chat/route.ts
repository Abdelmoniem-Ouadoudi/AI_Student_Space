...
function createMockResponse(query: string, locale: string = 'en') {
  const responses = {
    en: [
      ...
      `Thanks for your question: "${query}". I'm AI Student Space, your AI learning companion. This is a preview response since the backend service isn't currently available.`,
    ],
    fr: [
      ...
      `Merci pour votre question : "${query}". Je suis AI Student Space, votre compagnon d'apprentissage IA. Ceci est une réponse de prévisualisation car le service backend n'est pas actuellement disponible.`,
    ],
    ar: [
      ...
      `شكرًا لك على سؤالك: "${query}". أنا AI Student Space، رفيق التعلم بالذكاء الاصطناعي. هذه استجابة معاينة نظرًا لأن خدمة الواجهة الخلفية غير متاحة حاليًا.`,
    ]
  };
...
export async function POST(req: NextRequest) {
  ...
  // Use a default query if not provided
  const actualQuery = query || "Can you help me with these files?";
  ...
  try {    // Create a new FormData to send to backend with userId
    ...
    backendFormData.append("query", actualQuery);
    ...
    if (!backendRes.ok) {      const text = await backendRes.text();
      ...
      if (backendRes.status === 0 || text.includes("ECONNREFUSED")) {
        return createMockResponse(actualQuery, locale);
      }
