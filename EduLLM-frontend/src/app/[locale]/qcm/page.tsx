...
function QCMPageContent() {
  ...
  const handleBackToPrompts = () => {
    ...
    // Only return if no text AND no files
    if (!promptText.trim() && !attachedFiles?.length) return;
    
    // If we have files but no text, provide a default prompt
    let textToUse = promptText.trim();
    if (!textToUse && attachedFiles?.length) {
      textToUse = t("qcm.customPromptPlaceholder") || "Generate questions based on the uploaded materials";
    }    // Set processing state immediately to trigger UI change
    ...
    const enhancedPrompt = `${textToUse}\n\n${languageInstruction}`;
  ...
  const handlePDFGeneration = async (promptText: string, attachedFiles?: FileList) => {
    try {
      ...
      const query = promptText.trim() || t("qcm.customPromptPlaceholder") || "Generate questions based on the uploaded materials";
  ...
  // Quick prompts array removed as it's no longer needed
  return (
              ...
              <AnimatedSection delay={0.2}>
                <div className="text-center space-y-8">
                  ...
                  <AnimatedSection delay={0.4}>
                    <div className="flex items-center justify-center w-20 h-20 mx-auto mb-6 rounded-2xl overflow-hidden">
                      <img 
                      ...
                      <div className="w-full max-w-4xl mx-auto">                        <FileInputArea
                          ...
                          onSubmit={(e) => {
                            ...
                            if (files && files.length > 0) {
                              // Add a default query if no custom text is provided
                              const defaultQuery = t("qcm.customPromptPlaceholder") || "Generate questions based on the uploaded materials";
                              handleSubmit(e, {
                                experimental_attachments: files,
                                customText: defaultQuery // Always provide a default query
                              });
                            }
                          }}
                          files={files}
                        ...
                        <div className="flex flex-col items-center gap-3 mt-4">
                          {files && files.length > 0 && (
                            <div className="inline-flex items-center gap-2 px-3 py-1 bg-emerald-500/20 border border-emerald-500/30 rounded-full text-emerald-300 text-sm">
                              ...
                              <span>{files.length} file{files.length > 1 ? 's' : ''} ready - Upload complete</span>
                            </div>
                          )}
                          ...
                          {/* Add a button to submit files directly */}
                          {/* <button
                            type="button"
                            onClick={() => handlePromptClick(t("qcm.customPromptPlaceholder") || "Generate questions based on the uploaded materials")}
                            className="mt-4 w-full max-w-md bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-medium py-3 px-6 rounded-xl transition-all duration-200 flex items-center justify-center gap-2 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:ring-offset-2 focus:ring-offset-slate-900"
                          >
                            <FileText className="w-5 h-5" />
                            {files && files.length > 0 ? "Generate Questions from Uploaded Files" : "Enter Custom Query"}
                          </button> */}
                        </div>
                      </div>
                      {/* Quick prompts grid removed as requested */}
                    </div>
                  </AnimatedSection>
                </div>
              </AnimatedSection>            )}
              ...
              <div className="sticky bottom-0 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-t border-slate-700/50 p-4">
                <div className="max-w-7xl mx-auto">                  <FileInputArea
                    ...
                    onSubmit={(e) => {
                      ...
                      if (followUpInput?.trim() || files?.length) {
                        ...
                        // If there's no text but there are files, use a default prompt
                        const textToUse = followUpInput?.trim() || (files?.length ? t("qcm.customPromptPlaceholder") || "Generate questions based on the uploaded materials" : "");
                        const enhancedQuery = `${textToUse}\n\n${languageInstruction}`;
